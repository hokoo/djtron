...
## Что осталось закрыть (неполное покрытие)

- [ ] **A**: A1 (remote-доставка именно на Host для `play/stop`) и A3 (ограничение локального Simple для slave) не зафиксированы отдельными тестами.
- [ ] **B**: B1–B4 проверены не полностью (нет явной проверки seamless/playhead и кейса неактивного плейлиста).
- [ ] **C**: C2 (Play Next во время fragment применяется только после B) без отдельного теста.
- [ ] **D**: D2 (вставка после B в transition) и D5 (A отменяет `ScheduledSwitch`) без отдельного теста.
- [ ] **E**: E2/E3/E4 покрыты не полностью (transition-anchor, факт switch по `SEGMENT_ENDED`, сохранение `uiState` после switch).
- [ ] **F**: F1–F7 покрыты частично; сейчас есть только reset DAP при STOP.
- [ ] **G**: G3 (fade только тишина↔Track) без отдельного теста.
- [ ] **H**: H1 покрыт частично на уровне состояния (без отдельной проверки вызова `AudioEngine.stopAll`).
- [ ] **I**: I1 покрыт для copy; move-сценарий не зафиксирован отдельным тестом.
